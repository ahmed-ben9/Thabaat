...
const ERROR_TYPES = {
  nisyan:  {label:"Nisyan",  ar:"\u0646\u0633\u064a\u0627\u0646", desc:"Oubli",           color:"#ef4444", icon:"\uD83D\uDD34"},
...
const DUAS = [
  ...
  {ar:"\u0627\u0644\u0644\u0651\u064e\u0647\u064f\u0645\u0651\u064e \u0627\u0646\u0652\u0641\u064e\u0639\u0652\u0646\u0650\u064a \u0628\u0650\u0645\u064e\u0627 \u0639\u064e\u0644\u0651\u064e\u0645\u0652\u062a\u064e\u0646\u0650\u064a", fr:"Fais-moi profiter de ce que Tu m'as enseign\u00E9"},
  {ar:"\u0627\u0644\u0644\u0651\u064e\u0647\u064f\u0645\u0651\u064e \u0627\u0631\u0652\u0632\u064f\u0642\u0652\u0646\u0650\u064a \u062d\u0650\u0641\u0652\u0638\u064e \u0643\u0650\u062a\u064e\u0627\u0628\u0650\u0643\u064e", fr:"Accorde-moi la m\u00E9morisation de Ton Livre"},
  {ar:"\u0627\u0644\u0644\u0651\u064e\u0647\u064f\u0645\u0651\u064e \u0627\u062c\u0652\u0639\u064e\u0644\u0650 \u0627\u0644\u0652\u0642\u064f\u0631\u0652\u0622\u0646\u064e \u0631\u064e\u0628\u0650\u064a\u0639\u064e \u0642\u064e\u0644\u0652\u0628\u0650\u064a", fr:"Fais du Coran le printemps de mon c\u0153ur"},
];

const SECTIONS = {
  hifz:   {id:"hifz",   label:"Hifz",     labelAr:"\u0627\u0644\u062d\u0641\u0638",      subLabel:"Apprentissage", icon:"\u25C8", color:"#60a5fa", colorDim:"#60a5fa22", colorBorder:"#60a5fa44"},
  muraja: {id:"muraja", label:"Muraja'a", labelAr:"\u0627\u0644\u0645\u0631\u0627\u062c\u0639\u0629",  subLabel:"R\u00E9vision",     icon:"\u21BA", color:"#f59e0b", colorDim:"#f59e0b22", colorBorder:"#f59e0b44"},
  wird:   {id:"wird",   label:"Wird",     labelAr:"\u0627\u0644\u0648\u0631\u062f",      subLabel:"Lecture",       icon:"\u263D", color:"#4ade80", colorDim:"#4ade8022", colorBorder:"#4ade8044"},
};
  ...
  return (
    <svg style={{position:"fixed",inset:0,width:"100%",height:"100%",opacity:0.025,pointerEvents:"none",zIndex:0}} viewBox="0 0 600 600" preserveAspectRatio="xMidYMid slice">
      <defs><pattern id="g" x="0" y="0" width="80" height="80" patternUnits="userSpaceOnUse">
        ...
        <polygon points="40,14 66,28 66,52 40,66 14,52 14,28" fill="none" stroke={color} strokeWidth="0.3"/>
...
function SectionTabs({active,onChange}) {
  return (
    <div style={{display:"flex",gap:6,padding:"0 0",marginBottom:20}}>
      {Object.values(SECTIONS).map(s=>{
        const isActive=active===s.id;
        return (
          <button key={s.id} onClick={()=>onChange(s.id)} style={{
            flex:1,padding:"12px 4px",borderRadius:10,cursor:"pointer",
            border:isActive?`1px solid ${s.color}55`:"1px solid #1e1e1e",
            background:isActive?s.colorDim:"#111",
            display:"flex",flexDirection:"column",alignItems:"center",gap:2,
            transition:"all .2s",
          }}>
            <span style={{fontSize:18,color:isActive?s.color:"#333"}}>{s.icon}</span>
            <span style={{fontSize:10,fontWeight:700,color:isActive?s.color:"#444",letterSpacing:0.5}}>{s.label}</span>
  ...
  return (
    <div>
      <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>
        Versets ({Object.keys(verseErrors).length} erreurs)
      </div>
        ...
        <div style={{background:"#0d0d0d",border:"1px solid #c9a84c44",borderRadius:10,padding:14,marginBottom:10}}>
          ...
          {errorType==="tajweed"&&(
            <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
              {TAJWEED_RULES.map(r=>(
                <button key={r} onClick={()=>setTajweedRule(r)} style={{padding:"5px 10px",borderRadius:20,fontSize:11,cursor:"pointer",border:tajweedRule===r?"1px solid #a78bfa":"1px solid #222",background:tajweedRule===r?"#a78bfa22":"#111",color:tajweedRule===r?"#a78bfa":"#555"}}>{r}</button>
              ))}
            </div>
          )}
            ...
            return (
              <div key={v} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#111",border:"1px solid #1a1a1a",borderRadius:7,padding:"6px 10px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  ...
                  <span style={{fontSize:10,color:et.color,background:et.color+"22",borderRadius:20,padding:"1px 6px"}}>{et.icon} {et.label}</span>
                ...
                {!readOnly&&<button onClick={()=>{const n={...verseErrors};delete n[v];onChange(n);}} style={{background:"transparent",border:"none",color:"#444",cursor:"pointer",fontSize:16}}>x</button>}
              </div>
            );
          })}
        </div>
...
function CorrectorView({sessionId}) {
  ...
  if(loading) return <div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{color:"#c9a84c",fontFamily:"'Scheherazade New',serif",fontSize:24}}>\u0628\u0650\u0633\u0652\u0645\u0650 \u0627\u0644\u0644\u0651\u064e\u0647\u2026</div></div>;
  if(!session) return <div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12}}><div style={{fontSize:40}}>X</div><div style={{color:"#666",fontSize:14}}>Session introuvable</div></div>;
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0a0a0a",color:"#ddd",fontFamily:"'DM Sans',sans-serif",padding:"20px 16px 40px",maxWidth:500,margin:"0 auto"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{background:"#0d3320",border:"1px solid #4ade8033",borderRadius:10,padding:14,fontSize:12,color:"#4ade80",textAlign:"center"}}>Synchronisation en temps reel - Barakallahu fik</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{background:streakWarning?"linear-gradient(135deg,#1a0a0a,#2a1010)":"linear-gradient(135deg,#1a110a,#2a1a0a)",border:`1px solid ${streakWarning?"#ef444433":"#c9a84c33"}`,borderRadius:12,padding:"12px 16px",marginBottom:12,display:"flex",alignItems:"center",gap:12}}>
        <div style={{fontSize:26}}>{streakWarning?"...":"Fire"}</div>
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:10,color:streakWarning?"#ef444477":"#7a5a30",marginTop:1}}>{streakWarning?"Recite aujourd\'hui pour garder ton streak !":"Streak - Istimrariya"}</div>
        </div>
      </div>
      ...
      {inProgress>0&&(
        <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:12,padding:14,marginBottom:14}}>
          <div style={{fontSize:10,color:"#555",letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>En cours d\'apprentissage</div>
      ...
      <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:12,padding:14,marginBottom:14}}>
        <div style={{fontSize:10,color:"#555",letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Dernieres seances Hifz</div>
        ...
        recent.slice(0,4).map((s,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:i<3?"1px solid #1a1a1a":"none"}}>
            <div>
              ...
              <div style={{fontSize:10,color:"#444",marginTop:1}}>{s.date} - v.{s.range?.from} a v.{s.range?.to}</div>
            </div>
      ...
      <button onClick={onNewSession} style={{width:"100%",padding:13,background:`linear-gradient(135deg,${sec.colorDim},${sec.color}30)`,border:`1px solid ${sec.colorBorder}`,borderRadius:10,color:sec.color,fontSize:14,cursor:"pointer",fontWeight:600,letterSpacing:1}}>
...
function HifzSession({state,onSave,onDone}) {
  ...
  const newRanges=surah?[...existingRanges,{from:Math.min(rangeFrom,rangeTo),to:Math.max(rangeFrom,rangeTo)}]:existingRanges;
  ...
  const save=async()=>{
    ...
    await onSave(selSurah,session,allRanges);
    setSaved(true);setTimeout(onDone,1400);
  };

  if(saved) return <div style={{textAlign:"center",padding:60}}><div style={{fontSize:44}}>OK</div><div style={{color:sec.color,fontSize:16,marginTop:12}}>Seance enregistree !</div></div>;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:15}}>
      <div style={{textAlign:"center",marginBottom:4}}>
        ...
        <div style={{fontSize:11,color:"#555",marginTop:2}}>Nouvelle seance d\'apprentissage</div>
      </div>
      <div>
        <label style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:6}}>Sourate</label>
      ...
      {surah&&(
        <div style={{background:"#111",border:`1px solid ${sec.color}22`,borderRadius:10,padding:14}}>
          <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>Versets appris aujourd\'hui</div>
          {existingRanges.length>0&&(
            <div style={{marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#555",marginBottom:4}}>
                <span>Deja memorise</span><span style={{color:sec.color}}>{currentPct}%</span>
              </div>
              <div style={{height:3,background:"#1a1a1a",borderRadius:3,overflow:"hidden",marginBottom:6}}>
                <div style={{height:"100%",width:`${currentPct}%`,background:sec.color,borderRadius:3}}/>
              </div>
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                {existingRanges.map((r,i)=>(
                  <span key={i} style={{fontSize:9,color:sec.color+"88",background:sec.color+"11",border:`1px solid ${sec.color}22`,borderRadius:20,padding:"1px 7px"}}>v.{r.from} a {r.to}</span>
                ))}
              </div>
            </div>
          )}
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <div style={{flex:1}}>
              <div style={{fontSize:9,color:"#444",marginBottom:4,textTransform:"uppercase"}}>Du verset</div>
            ...
            <div style={{color:"#333",marginTop:14,fontSize:18}}>to</div>
            <div style={{flex:1}}>
              <div style={{fontSize:9,color:"#444",marginBottom:4,textTransform:"uppercase"}}>Au verset</div>
          ...
          {rangeFrom<=rangeTo&&(
            <div style={{marginTop:10,fontSize:11,color:sec.color+"99",textAlign:"center",background:sec.color+"11",borderRadius:7,padding:8}}>
              {rangeTo-rangeFrom+1} versets - Nouveau total: {newPct}% de {surah.name}
            </div>
          )}
        </div>
      )}
      <div>
        <label style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:6}}>Type de seance</label>
        <div style={{display:"flex",gap:8}}>
        ...
        {type==="sheikh"&&<input value={partner} onChange={e=>setPartner(e.target.value)} placeholder="Nom du cheikh" style={{width:"100%",background:"#111",border:"1px solid #2a2a2a",borderRadius:8,color:"#ddd",padding:"10px 12px",fontSize:13,outline:"none",marginTop:8,boxSizing:"border-box"}}/>}
      </div>
      ...
      {surah&&(
        <div style={{background:"#111",border:"1px solid #1e1e1e",borderRadius:10,padding:14}}>
          <VerseErrorPicker totalVerses={surah.v} verseErrors={verseErrors} onChange={setVerseErrors}/>
        </div>
      )}
      ...
      <button onClick={save} disabled={!selSurah} style={{padding:13,borderRadius:10,fontSize:14,fontWeight:600,letterSpacing:1,cursor:selSurah?"pointer":"not-allowed",background:selSurah?`linear-gradient(135deg,${sec.colorDim},${sec.color}40)`:"#111",border:selSurah?`1px solid ${sec.colorBorder}`:"1px solid #1a1a1a",color:selSurah?sec.color:"#333"}}>
    ...
    <div>
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        ...
        <select value={filter} onChange={e=>setFilter(e.target.value)} style={{background:"#111",border:"1px solid #222",borderRadius:8,color:"#888",padding:"8px",fontSize:11,outline:"none"}}>
          ...
          <option value="none">Non commencees</option>
        </select>
      </div>
            ...
            <div key={s.n} style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:10,padding:13}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:pct>0?8:0}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <span style={{fontFamily:"'Scheherazade New',serif",fontSize:13,color:"#c9a84c33",marginLeft:6}}>{s.ar}</span>
                  <div style={{fontSize:9,color:"#444",marginTop:1}}>{s.v} versets</div>
                </div>
                <div style={{fontSize:16,fontWeight:700,color:pct===100?sec.color:pct>0?"#f59e0b":"#2a2a2a",fontFamily:"monospace"}}>{pct}%</div>
              </div>
              {pct>0&&(
                <>
                  <div style={{height:3,background:"#1a1a1a",borderRadius:3,overflow:"hidden",marginBottom:5}}>
                    <div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${sec.color},${sec.color}88)`,borderRadius:3}}/>
                  </div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                    {ranges.map((r,i)=><span key={i} style={{fontSize:9,color:sec.color+"88",background:sec.color+"11",border:`1px solid ${sec.color}22`,borderRadius:20,padding:"1px 7px"}}>v.{r.from} to {r.to}</span>)}
                  </div>
                </>
              )}
            </div>
          ...
          return (
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:i<4?"1px solid #1a1a1a":"none"}}>
              <div>
                ...
                <div style={{fontSize:10,color:"#444",marginTop:1}}>{s.date}{s.duration?` - ${s.duration}min`:""} {s.quality?` - ${"*".repeat(s.quality)}`:""}</div>
              </div>
...
function MurajaSession({state,onSave,onDone}) {
  ...
  const save=async()=>{
    const dur=Math.round((Date.now()-startTime)/60000);
    const session={date:today(),type,partner:type==="solo"?null:partner,quality,notes,duration:dur||1,
      range:{fromSurah,fromVerse,toSurah,toVerse,fromName:fsData?.name,toName:tsData?.name}};
    await onSave(session);setSaved(true);setTimeout(onDone,1400);
  };

  if(saved) return <div style={{textAlign:"center",padding:60}}><div style={{fontSize:44}}>OK</div><div style={{color:sec.color,fontSize:16,marginTop:12}}>Muraja\'a enregistree !</div></div>;

  return (
    <div style={{display:"flex",flexDirection:"column",gap:15}}>
      <div style={{textAlign:"center",marginBottom:4}}>
        <div style={{fontFamily:"'Scheherazade New',serif",fontSize:22,color:sec.color}}>\u062a\u0633\u062c\u064a\u0644 \u0645\u0631\u0627\u062c\u0639\u0629</div>
        <div style={{fontSize:11,color:"#555",marginTop:2}}>Nouvelle seance de revision</div>
      </div>
      <div style={{background:"#111",border:`1px solid ${sec.color}22`,borderRadius:10,padding:14}}>
        <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>Plage revisee</div>
        {totalInRange>0&&(
          <div style={{marginBottom:12,background:sec.color+"11",border:`1px solid ${sec.color}22`,borderRadius:8,padding:"8px 12px",fontSize:11,color:sec.color+"cc",textAlign:"center"}}>
            {fsData?.name} {fromVerse} - {tsData?.name} {toVerse} ({totalInRange} versets, {pct}% du Coran)
          </div>
        )}
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:8,alignItems:"start"}}>
          <div style={{background:"#0d0d0d",border:`1px solid ${sec.color}22`,borderRadius:10,padding:12}}>
            <div style={{fontSize:9,color:sec.color+"88",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Debut</div>
          ...
          <div style={{color:"#333",fontSize:22,paddingTop:36,textAlign:"center"}}>to</div>
          <div style={{background:"#0d0d0d",border:`1px solid ${sec.color}22`,borderRadius:10,padding:12}}>
            <div style={{fontSize:9,color:sec.color+"88",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Fin</div>
            <div style={{marginBottom:8}}>
              <div style={{fontSize:9,color:"#444",marginBottom:4}}>Sourate</div>
      ...
      <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:10,padding:14}}>
        <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>Qualite de la revision</div>
        <div style={{display:"flex",gap:6}}>
          {[1,2,3,4,5].map(q=>(
            <button key={q} onClick={()=>setQuality(q)} style={{flex:1,padding:"10px 4px",borderRadius:8,fontSize:18,cursor:"pointer",border:quality>=q?`1px solid ${sec.color}55`:"1px solid #1a1a1a",background:quality>=q?sec.colorDim:"#0d0d0d",color:quality>=q?sec.color:"#2a2a2a"}}>*</button>
          ))}
        </div>
      ...
      <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={2} placeholder="Points a ameliorer..." style={{width:"100%",background:"#111",border:"1px solid #2a2a2a",borderRadius:8,color:"#ddd",padding:"10px 12px",fontSize:13,outline:"none",resize:"vertical",boxSizing:"border-box",fontFamily:"inherit"}}/>
      <button onClick={save} style={{padding:13,borderRadius:10,fontSize:14,fontWeight:600,letterSpacing:1,cursor:"pointer",background:`linear-gradient(135deg,${sec.colorDim},${sec.color}40)`,border:`1px solid ${sec.colorBorder}`,color:sec.color}}>
        Enregistrer la revision
      </button>
    </div>
  );
}
      ...
      <div style={{background:"linear-gradient(135deg,#001a0a,#002510)",border:`1px solid ${sec.color}22`,borderRadius:12,padding:16,marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
          <div>
            ...
            <div style={{fontSize:10,color:sec.color+"66",marginTop:2,textTransform:"uppercase",letterSpacing:1}}>
        ...
        <div style={{marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,marginBottom:5}}>
            <span style={{color:"#555"}}>Progression</span>
        ...
        {lastSession&&(
          <div style={{background:"#ffffff08",borderRadius:8,padding:"8px 12px",fontSize:11,marginTop:8}}>
            <span style={{color:"#444"}}>Derniere position: </span>
            <span style={{color:sec.color,fontWeight:600}}>{SURAHS.find(s=>s.n===lastSession.toSurah)?.name} - verset {lastSession.toVerse}</span>
          </div>
        )}
      </div>
      <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:12,padding:14,marginBottom:14}}>
        ...
        [...sessions].sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,6).map((s,i)=>{
          ...
          return (
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:i<5?"1px solid #1a1a1a":"none"}}>
              <div>
                <div style={{fontSize:12,color:"#ccc"}}>{s.mode==="point"?`Jusqu\'a ${tName} ${s.toVerse}`:`${fName} ${s.fromVerse} to ${tName} ${s.toVerse}`}</div>
...
function WirdSession({state,onSave,onDone}) {
  ...
  if(saved) return <div style={{textAlign:"center",padding:60}}><div style={{fontSize:44}}>OK</div><div style={{color:sec.color,fontSize:16,marginTop:12}}>Lecture enregistree !</div></div>;

  return (
    <div style={{display:"flex",flexDirection:"column",gap:15}}>
      <div style={{textAlign:"center",marginBottom:4}}>
        <div style={{fontFamily:"'Scheherazade New',serif",fontSize:22,color:sec.color}}>\u062a\u0633\u062c\u064a\u0644 \u0648\u0631\u062f</div>
        <div style={{fontSize:11,color:"#555",marginTop:2}}>Enregistrer une lecture</div>
      </div>
      {lastSession&&(
        <div style={{background:sec.colorDim,border:`1px solid ${sec.colorBorder}`,borderRadius:8,padding:"10px 14px",fontSize:11}}>
          <span style={{color:"#555"}}>Derniere position: </span>
          <span style={{color:sec.color,fontWeight:600}}>{SURAHS.find(s=>s.n===lastSession.toSurah)?.name} - verset {lastSession.toVerse}</span>
        </div>
      )}
      <div>
        <label style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:6}}>Mode de saisie</label>
        <div style={{display:"flex",gap:8}}>
      ...
      {mode==="range"&&(
        <div style={{background:"#111",border:`1px solid ${sec.color}22`,borderRadius:10,padding:14}}>
          <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>Debut de la lecture</div>
      ...
      <div style={{background:"#111",border:`1px solid ${sec.color}22`,borderRadius:10,padding:14}}>
        <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:10}}>
          {mode==="point"?"Je me suis arrete a...":"Fin de la lecture"}
        </div>
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:2}}>
            <div style={{fontSize:9,color:"#444",marginBottom:4}}>Sourate</div>
  ...
  return (
    <div>
      <div style={{textAlign:"center",marginBottom:16}}>
        ...
        <div style={{fontSize:11,color:"#555",marginTop:2}}>Classement des freres</div>
      </div>
      ...
      {!state.profile?.name&&<div style={{background:"#1a1a0a",border:"1px solid #f59e0b33",borderRadius:10,padding:12,marginBottom:14,fontSize:12,color:"#f59e0b"}}>Configure ton prenom dans Profil pour apparaitre ici.</div>}
        ...
        {friends.map((f,i)=>(
          <div key={f.uid} style={{background:f.uid===UID?"#111822":"#111",border:`1px solid ${f.uid===UID?"#60a5fa33":"#1a1a1a"}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:10,color:"#444",marginTop:2}}>Streak: {f.streak||0} jours</div>
            </div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{textAlign:"center",marginBottom:4}}>
        ...
        <div style={{fontSize:11,color:"#555",marginTop:2}}>Profil and Parametres</div>
      </div>
      <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:12,padding:16}}>
        <label style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:8}}>Ton prenom</label>
      ...
      <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:12,padding:16}}>
        <div style={{fontSize:10,color:"#555",letterSpacing:1,textTransform:"uppercase",marginBottom:12}}>Rappels</div>
        {[{key:"hifz",title:"Rappel Hifz",icon:"H",color:"#60a5fa"},{key:"muraja",title:"Rappel Muraja\'a",icon:"R",color:"#f59e0b"},{key:"wird",title:"Rappel Wird",icon:"W",color:"#4ade80"}].map(rem=>{
          ...
          return (
            <div key={rem.key} style={{marginBottom:14,paddingBottom:14,borderBottom:"1px solid #1a1a1a"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:r.enabled?10:0}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:14,color:rem.color}}>{rem.icon}</span>
